...
namespace Gf.Frs.LoaderWcfServices.Bank.MT940
{
    ...
    {
        public IAsyncResult BeginMT940Load(LoadMT940Request request, AsyncCallback callback, object state)
        {
            LoadMT940Response response = LoadMT940(request);

            var asyncResult = new LoadMT940AsyncResponse(response.Code, response.Message, callback, state);
        ...
        public IAsyncResult EndMT940Load(IAsyncResult asyncResult)
        {
            var operationAsyncResult = asyncResult as LoadMT940AsyncResponse;
            operationAsyncResult.AsyncWait.WaitOne();
            return operationAsyncResult;
        }

        private LoadMT940Response LoadMT940(LoadMT940Request request)
        {
        ...
        {
            var asyncResult = state as LoadMT940AsyncResponse;
