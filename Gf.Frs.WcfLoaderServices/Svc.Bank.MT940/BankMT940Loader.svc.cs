    ...
    public class BankMT940Loader : IBankMT940Loader, IDisposable
    {
        ...
        public LoadMT940Response LoadMT940(LoadMT940Request request)
        {
            ...
            _logManager.SetSPDetailsAndLog(LogLevel.Info, string.Format("{2}{0}{2}{0}{1}{0}{2}{0}{2}",
                                          Environment.NewLine,
                                          ("Entering LoadMT940 and setting up logger configurations... " + _currentLogerName),
                                          "#########################################################################################################"));
            #endregion **LOG ENTRY**
            ...
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" Starting validation... Validating input (LoadMT940Request) request.");
            #endregion **LOG ENTRY**

            if (request == null)
            {
                _logManager.RaiseException(string.Format("[a]{2} There was a fault validating the request.{0}Fault details:{0}{1}",
                                            Environment.NewLine,
            ...
            if (request.LoadId <= 0)
            {
                _logManager.RaiseException(string.Format("[b]{3} There was a fault validating passed Load Id = {1}.{0}Fault details:{0}{2}",
                                            Environment.NewLine,
            ...
            {
                _logManager.RaiseException(string.Format("[c]{3} There was a fault validating passed User Id = {1}.{0}Fault details:{0}{2}",
                                            Environment.NewLine,
                                            request.UserId,
            ...
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" Validating input (LoadMT940Request) request completed successfully.");
            #endregion **LOG ENTRY**
            ...
            {
                #region **LOG ENTRY**
                _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" Validate the Load object fully, including the associated objects.");
                #endregion **LOG ENTRY**
            ...
            {
                _logManager.RaiseException(string.Format("[d]{3} MT940 unexpected error! during validation of Load Id = {1} / [5].{0}Fault details:{0}{2}",
                                          Environment.NewLine,
                                          request.LoadId,
            ...
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" Get load object from database (to be used through out the function for this call), i.e. requested load.");
            #endregion **LOG ENTRY**
            ...
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+string.Format(" Load (Load Id = {0}), successfully found in the database.", request.LoadId));
            #endregion **LOG ENTRY**

            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" Loading reference data for the operation.");
            #endregion **LOG ENTRY**
            ...
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" Reference data loaded successfully.");
            #endregion **LOG ENTRY**

            #endregion

            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" Validating the load record against reference data i.e. Read-only & Load Status.");
            #endregion **LOG ENTRY**
            ...
            if (dbLoad.ReadOnly || dbLoad.LoadStatusId != refLoadStatuses.Find(rls => rls.Name.ToLower().Equals(RefDataLoadStatus.LSSubmitted)).Value)
            {
                ...
                _logManager.RaiseException(string.Format("[f]{3} Error! during MT940 load checking of Load Id = {1} / [7].{0}Fault details:{0}{2}",
                                                        Environment.NewLine,
                                                        request.LoadId,
            ...
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" Reference data validation completed successfully.");
            #endregion **LOG ENTRY**
            ...
            _mt940LoadHndlr.UpdateLoadStatusInNewContext(request.LoadId,
                                                        ...
                                                        true);
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" MT940 Load status set to 'Parsing'.");
            #endregion **LOG ENTRY**

            #region  ##START## MT940 Header and Trailer setup

            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" MT940 header and trailer setup starting...");
            #endregion **LOG ENTRY**
            ...
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" MT940 header and trailer completed successfuly.");
            #endregion **LOG ENTRY**
            ...
            {
                #region **LOG ENTRY**
                _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" Validating the MT940 file contents.");
                #endregion **LOG ENTRY**
                ...
                #region **LOG ENTRY**
                _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" MT940 file contents validated successfully.");
                #endregion **LOG ENTRY**
                ...
                #region **LOG ENTRY**
                _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" MT940 Load record status set to 'Transforming'.");
                #endregion **LOG ENTRY**
            }
            catch (Exception ex)
            {
                ...
                _logManager.RaiseException(string.Format("[g]{3} MT940 unexpected error! during MT940 file contents validation of Load Id = {1} / [8].{0}Fault details:{0}{2}",
                                            Environment.NewLine,
                                            request.LoadId,
                                            ExceptionExtensions.ToDetailedString(ex),
                                            _currentLogerName));
            ...
            if (faults != null && faults.Count > 0)
            {
                ...
                _logManager.RaiseException(string.Format("[h]{3} There was a fault validating passed Load Id = {1} / [9], associated MT940 content.{0}Fault details:{0}{2}",
                                                        Environment.NewLine,
                                                        request.LoadId.ToString(),
                                                        DotNetHelper.WrapFaultListToString(faults),
                                                        _currentLogerName));
            ...
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" MT940 loading started...");
            #endregion **LOG ENTRY**
            ...
            try
            {
                ...
                _mt940LoadHndlr.UpdateLoadStatusInNewContext(request.LoadId,
                                                            ...
                                                            true);
                #region **LOG ENTRY**
                _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" MT940 Load record status set to 'Importing'.");
                #endregion **LOG ENTRY**
                ...
                #region **LOG ENTRY**
                _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" MT940 data loaded in MT940 entries table.");
                #endregion **LOG ENTRY**
            }
            catch (Exception ex)
            {
                ...
                _logManager.RaiseException(string.Format("[i]{3} MT940 unexpected error! during the load processing into database of Load Id = {1} / [10].{0}Fault details:{0}{2}",
                                            Environment.NewLine,
                                            request.LoadId,
                                            ExceptionExtensions.ToDetailedString(ex),
                                            _currentLogerName));
            ...
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" MT940 data load complete.");
            #endregion **LOG ENTRY**
            ...
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" MT940 load house keeping started...");
            #endregion **LOG ENTRY**
            ...
            catch (Exception ex)
            {
                ...
                _logManager.RaiseException(string.Format("[j]{3} MT940 unexpected error! after load completion of Load Id = {1} / [11], while updating the load record for progress.{0}Fault details:{0}{2}",
                                            Environment.NewLine,
                                            request.LoadId,
                                            ExceptionExtensions.ToDetailedString(ex),
                                            _currentLogerName));
            ...
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" MT940 load house keeping completed successfully.");
            #endregion **LOG ENTRY**
            ...
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+string.Format(" MT940 load for Load Id = {0} completed successfully. Time taken - {1}", request.LoadId, elapsedTime), response.ToString());
            #endregion **LOG ENTRY**
        ...
        private void LoadFailed(LoadMT940Request request, List<RefDataLoadStatus> refLoadStatuses)
        {
            ...
            _mt940LoadHndlr.UpdateLoadStatusInNewContext(request.LoadId,
                                                        ...
                                                        true);
            #region **LOG ENTRY**
            _logManager.LogFromSPDetails(LogLevel.Info,  _currentLogerName+" MT940 Load record status set to 'Failed'.");
            #endregion **LOG ENTRY**
        }
